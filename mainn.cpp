...
int input_int(const char *prompt) {
    ...
    printf("%s", prompt);
    if (scanf("%d", &val) != 1) {
        // Invalid input, clear buffer and return a default or error value
        while (getchar() != '\n');
...
bool update_employee(const Employee *e, const char *filename) {
    ...
    Employee temp;
    long pos;
    while ((pos = ftell(file)) != -1 && fread(&temp, sizeof(Employee), 1, file) == 1) {
        if (temp.id == e->id) {
...
void list_employees(const char *filename) {
    ...
    while (fread(&e, sizeof(Employee), 1, file) == 1) {
        printf("ID: %d, Name: %s, Basic: %.2f, Allowances: %.2f, Deductions: %.2f, Gross: %.2f, Net: %.2f\n",
               e.id, e.name, e.basic, e.allowances, e.deductions, e.gross, e.net);
    }
    fclose(file);
}
...
void do_update() {
    ...
    input_string("Enter new name (leave blank to keep): ", name_buffer, NAME_LEN);
    ...
    printf("Enter new allowances (current %.2f): ", e.allowances); 
    if (scanf("%lf", &temp_d) == 1) e.allowances = temp_d;
    
    printf("Enter new deductions (current %.2f): ", e.deductions); 
